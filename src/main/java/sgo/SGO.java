...
public class SGO {
    ...
    public SGO(int playerNumber, int substituteNumber, int kicksLimit, Function function) {
        if(rand == null) {
            rand = new Random();
        }
    ...
    public SGO(int playerNumber, int substituteNumber, int kicksLimit, double inertiaWeight, 
            double cognitiveWeight, double socialWeight, Function function,
            double beginRange, double endRange, double moveOffProbability, double moveForwardAfterMoveOffProbability,
            double substitutionProbability, double nonUniformityDegree) {
        if(rand == null) {
            rand = new Random();
        }
    ...
    public Player[] initialize() {
        ...
        for(int i = 0; i < this.playerNumber; ++i) {
            players[i] = new Player(this.function, this.beginRange, this.endRange, rand);
    ...
    public double[] moveOff(double[] position, int kick) {
        ...
        for(int i = 0; i < position.length; ++i) {
            if(rand.nextDouble() <= 0.5) {
                newPosition[i] = position[i] + NUM(kick, (this.endRange - position[i]));
            } else {
                newPosition[i] = position[i] - NUM(kick, (position[i] - this.beginRange));
            }
        }
        return newPosition;
    }
